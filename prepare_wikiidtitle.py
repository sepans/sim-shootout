...
* lsi.model: model for latent semantic analysis model, trained on TF-IDF'ed wiki dum
...
PROCESSES = max(1, multiprocessing.cpu_count() - 3)  # parallelize parsing using this many processes
...
def process_article((title, text, pageid)):
    ...
    return pageid, gensim.utils.to_unicode(title).replace('\t', ' '), gensim.utils.simple_preprocess(text)
...
def convert_wiki(infile, processes=multiprocessing.cpu_count()):
    ...
    for group in gensim.utils.chunkize(texts, chunksize=10 * processes):
        for pageid, title, tokens in pool.imap(process_article, group):
...
if __name__ == '__main__':
    ...
    if not os.path.exists(preprocessed_file):
        id2title = [] #collections.OrderedDict([]) 
        with gensim.utils.smart_open(preprocessed_file, 'wb') as fout:
            for docno, (pageid, title, tokens) in enumerate(convert_wiki(infile, PROCESSES)):
                id2title.append(pageid)
        ...
        gensim.utils.pickle(id2title, outfile('wikiidlist'))
